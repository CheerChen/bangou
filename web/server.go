...
var (
	...
	partialTemplates  = template.Must(template.ParseFS(content, "templates/group.html", "templates/index.html"))
...
func NewServer(stg *staging.Manager, store committed.Store, exec *executor.Executor, scanFn func(), rescrapeFn func(string), aria2 Aria2Status) http.Handler {
	h := &Handlers{staging: stg, store: store, executor: exec, scanFn: scanFn, rescrapeFn: rescrapeFn, aria2: aria2}
