    ...
    <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl overflow-hidden hover:border-gray-700 transition">
        ...
        <div className="relative aspect-[16/9] overflow-hidden bg-black">
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between">
            <div>
              ...
              {item.rating && (
                <span className="ml-2 text-xs text-amber-400">★ {item.rating}{item.reviewCount > 0 && ` (${item.reviewCount})`}</span>
              )}
            </div>
            {item.alive
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="text-xs text-gray-600 space-y-0.5">
          <div>
            <span className="text-gray-500">{item.linkType}</span>
            {item.srcPath && <span>: <code className="text-gray-500">{item.srcPath}</code></span>}
          </div>
        ...
        <div className="flex gap-1.5 pt-2 border-t border-gray-800">
          ...
          <button className="flex items-center gap-1 text-xs px-2.5 py-1.5 text-gray-500 hover:text-amber-400 hover:bg-[#222] rounded-lg transition" title="Unlink">
            <Unlink size={12} />Unlink
          </button>
          <button className="flex items-center gap-1 text-xs px-2.5 py-1.5 text-gray-500 hover:text-red-400 hover:bg-[#222] rounded-lg transition" title="Remove">
